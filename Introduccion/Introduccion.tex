...
Por eso el término proteoma es más flexible y puede ajustarse a definiciones más específicas.
...
\figuraEx{Vectorial/proteome_complexity}{width=.80\textwidth}{fig:proteome_complexity}%
{Aumento de complejidad desde el genoma hacia el proteoma. El Genoma, el conjunto de genes
de una célula, es el mismo, fijo, para todas las células del organismo, mientras que el Proteoma
es más flexible y variable ya que implica varios niveles de complejidad adicional. }
...
como la desorción tipo \ac{FD}, la desorción \ac{PD} y la ionización \ac{FAB} \citep{Roepstorff1992} permitió por primera vez que las
...
Otros tipos de ionización menos empleados en proteómica son los mencionados 
\acs{FAB}, \acs{FD} y \acs{PD}.


\figuraEx{Vectorial/maldi_y_esi}{width=.60\textwidth}{fig:maldi_y_esi}%
{Ionización MALDI (a) y ESI (b), dos de los tipos de fuentes de ionización más frecuentemente
empleados en espectrometría de masas para proteómica.}
...
\figuraEx{Vectorial/Q_QIT_OT}{width=.93\textwidth}{fig:Analizadores de masas}%
...
\figuraEx{Vectorial/tandem_ms}{width=.92\textwidth}{fig:tandem_ms}%
...
\figura{Vectorial/roepstorff_nomenclature}{width=.56\textwidth}{fig:peptide_fragmentation}%
...
\figuraEx{Vectorial/overview_of_shotgun_proteomics}{width=.90\textwidth}{fig:overview_of_shotgun_proteomics}%
...
A diferencia de la técnica de la huella de masaas peptídicas donde cada proteína se encuentra
...
superior, por ejemplo, por fraccionamientos subcelulares correspondientes a distintos 
orgánulos.
...
por la fase móvil y se separan de la fase estacionaria. El tiempo
de retención o tiempo de elución es el tiempo que necesita un
...
\figuraEx{Vectorial/spectrum_interpret_from_man_to_auto_2}{width=.66\textwidth}{fig:spectrum_interpretation_2}%
{Interpretación automática de espectros MS/MS. 
En primer lugar se elabora una lista de péptidos candidatos que pueden ser potencialmente responsables
del espectro adquirido. Para ello el espacio de búsqueda en la base de datos de secuencias se puede
acotar conociendo el tipo de enzima proteolítica utilizada o la tolerancia (diferencia
entre masa experimental y teórica) permitida entre otros parámetros suministrados al motor de búsqueda.
Conociendo además una probabilidad estimada de que cada tipo de ión se encuentre en un espectro MS/MS
se puede crear un espectro teórico para cada uno de los péptidos candidatos de la lista elaborada.
A continuación, cada uno de los espectros teóricos es comparado con el espectro empírico, se obtienen todos los 
iones coincidentes y se asigna una puntuación. Finalmente, en base a la puntuación, se obtiene
una lista de los mejores péptidos candidatos que presumiblemente han generado el espectro.}
{Interpretación automática de espectros MS/MS}
...
intensidad son eliminados y el resto de valores \mz son redondeados al valor
...
\figuraEx{Vectorial/tabla_contingencia_estadistica_basica}{width=.98\textwidth}{fig:tabla_contingencia_estadistica_basica}%
...
\figuraEx{Vectorial/evalue_estimation}{width=.86\textwidth}{fig:evalue_estimation}%
...
\figuraEx{Vectorial/single_spectrum_and_average_distributions}{width=0.99\textwidth}{fig:single_spectrum_and_average_distributions}%
{La distribución de espectro individual es la distribución de las puntuaciones
de todos los péptidos candidatos comparados con el espectro empírico y permite obtener
los parámetros estadísticos e-valor y p-valor. 
La distribución promedio es la de las puntuaciones de los mejores candidatos 
para el total de todos los espectros adquiridos y permite obtener una evaluación
estadística a nivel global, bien mediante la estimación de la tasa FDR o bien mediante
modelos estadísticos más complejos como el implementado en la herramienta PeptideProphet (modelo
mixto de probabilidad).}
...
motivo tradicionalmente se han utilizado estrategias de \emph{corrección de test múltiple} 
...
esencialmente en utilizar una base de datos llamada señuelo
...
sean comparados con espectros teóricos derivados de secuencias señuelo,
...
estas secuencias señuelo permite %es % permiten %por tanto
recrear una hipótesis nula. Se puede tener la certeza de que los resultados 
de identificaciones correspondientes a secuencias señuelo, claramente 
...
un número equivalente de secuencias señuelo y hacer que el motor de búsqueda
use esta base de datos concatenada (secuencias reales - secuencias señuelo)
del doble de tamaño que la original.
O bien se pueden realizar dos búsquedas consecutivas, una utilizando la
base de datos de secuencias reales y otra a continuación utilizando
la de secuencias señuelo.
...
Las secuencias señuelo pueden obtenerse mediante
...
las mismas secuencias señuelo para sucesivas búsquedas. A cambio, 
el hecho de que no sea un orden aleatorio puede implicar
que la población señuelo no refleje exactamente una hipótesis nula.
...
crear péptidos señuelo de cada proteína dado el patrón de corte
...
señuelo sigue la misma distribución que la frecuencia con que los
espectros son asignados incorrectamente a secuencias reales. 
...
incorrectamente una secuencia real)
puede ser considerado equivalente al número de \ac{PSM} señuelo 
(N\textsubscript{d}, espectros a los que se ha asginado una secuencia señuelo).
Con esto se puede estimar la tasa FDR como
 N\textsubscript{d}/N\textsubscript{t},
esto es, la proporción de \ac{PSM} señuelo, N\textsubscript{d} como sustituto
conocido de N\textsubscript{inc}, 
entre el total de secuencias reales con puntuaciones superiores
...
la base de datos de secuencias reales y a continuación sobre la equivalente de secuencias señuelo,
...
se compara con las secuencias señuelo a pesar de que algunos de los
espectros podrían asignarse correctamente a una secuencia real.
Además, la mayoría de las funciones de puntuación tienden a otorgar
puntuaciones más altas a \ac{PSM} señuelo que a \ac{PSM} reales
incorrectos %\citep{Navarro2009a}
por lo que la distribución de puntuaciones
señuelo no es un reflejo preciso de la distribución de puntuaciones
...
por el porcentaje estimado de \ac{PSM} reales pero incorrectos (PIT): Si en 
...
para obtener un valor FDR más preciso (Por cada 100 \ac{PSM} señuelo
en el conjunto de \ac{PSM} aceptado se estiman 80 \ac{PSM} reales incorrectos) 


Las búsquedas utilizando una base de datos concatenada 
son menos sensibles al efecto de sobre-estimación de 
...
secuencias reales y señuelo por separado sino simultáneamente
lo que produce un efecto de competición.
Se puede considerar que las secuencias reales y señuelo
...
secuencia real correcta. En tal caso se produce un aumento 
...
de poblaciones de \ac{PSM} reales y \ac{PSM} señuelo y corrige
...
Brevemente, PeptideProphet recalcula las puntuaciones y emplea un 
...
una distribución \textit{Normal}({\large$\mu$, $\sigma$}), y las asignaciones incorrectas,
una distribución \textit{Gamma}({\large$\alpha$, $\beta$, $\gamma$})
...
Para iniciar, el algoritmo require los parámetros {\large$\pi_{0}$} (la proporción
de asignaciones incorrectas en toda la población); {\large$\mu$, $\sigma$}, parámetros
que definen la \textit{Normal}; y {\large$\alpha$, $\beta$, $\gamma$}, que definen la \textit{Gamma}.
...
en valor absoluto de un error predefinido suministrado, {\large$\epsilon$}.
...
\figuraEx{Vectorial/protein_inference_non_random_peptide_grouping}{width=.90\textwidth}{fig:protein_inference_non_random_peptide_grouping}%
...
son las distribuciones NSP entre los péptidos incorrectos y correctos, p$'$\textsubscript{i} 
...
que permite ajustar la contribución de un péptido a su proteína en función de si la
secuencia es exclusiva o está presente en varias proteínas.
...
\figuraEx{Vectorial/esquema_srm}{width=.99\textwidth}{fig:esquema_srm}%
{Adquisición y reconstrucción de la señal en un experimento SRM.
Las técnicas \ac{SRM} se efectúan en instrumentos de tipo triple cuadrupolo.
El péptido monitorizado es seleccionado en el primer cuadrupolo, filtrando el resto 
de masas. A continuación es fragmentado en el segundo cuadrupolo y en el último
cuadrupolo se monitorizan los fragmentos. La señal es registrada como un pico
de elución en el que las señales de cada fragmento idealmente han de coeluir, 
confirmando que corresponden al mismo péptido precursor.}
{Adquisición y reconstrucción de la señal en un experimento SRM.} 
...
de valores \mz de un precursor y valores \mz de sus fragmentos correspondientes.
...
\subsection{GPMDB}
\label{GPMDB}
El sistema \ac{GPMDB} \citep{Craig2004} almacena resultados
de experimentos de proteómica, recogidos de otros repositorios públicos, en forma de ficheros XML indexados
en una base de datos mediante un sistema denominado XIAPE (\textit{Xml 
Information About a Proteomics Experiment}).
Parte de los resultados del PeptideAtlas de \textit{\mbox{C. albicans}} descritos
en esta tesis son accesibles \href{http://www.thegpm.org/dsotw_2014.html#201406151}{desde GPMDB}
\newline
(\href{http://www.thegpm.org/dsotw_2014.html#201406151}{http://www.thegpm.org/dsotw\_2014.html\#201406151}).

%-------------------------------------------------------------------
%-------------------------------------------------------------------

%\phantomsection
\section{Formatos de archivos usados frecuentemente en \\ espectrometría de masas y proteómica}
...
\textit{\mbox{C. albicans}} es un hongo unicelular diploide de la clase \textit{Saccharomycetes}, patógeno oportunista que se encuentra comúnmente
...
%Mayer Hube 2013 \mbox{C. albicans} pathogenicity mechanisms:

Los principales factores de virulencia con los que cuenta \textit{\mbox{C. albicans}}
para proliferar y diseminarse causando infecciones son su polimorfismo,
su capacidad de producir adhesinas e invasinas, %
la secreción de enzimas hidrolíticas %
y la formación de biopelículas \citep{Mayer2013}. 
...
como Als3p \citep{Phan2007, DeGroot2013} y otras como la también anclada mediante GPI y asociada
a hifas Hwp1p \citep{Sundstrom2002} son algunas de las adhesinas más estudiadas.

A continuación de la adhesión a la superficie celular del hospedador, la secreción de enzimas hidrolíticas
como proteasas (como las de la familia SAP), fosfolipasas y lipasas tiene un papel importante 
en la virulencia. Se ha descrito que pueden facilitar la penetración en las células \citep{Wachtler2012} 
e incluso podrían mejorar la eficacia en la adquisición de nutrientes extracelulares \citep{Naglik2003}
...
un factor de virulencia crítico. Se ha observado que biopelículas maduras
...
{La población comensal de células de \textit{\mbox{C. albicans}}
...
estudios usando el modelo de \textit{\mbox{C. albicans}}. Existen
estudios proteómicos de la interacción con el hospedador, tanto desde el enfoque de
\textit{\mbox{C. albicans}} \citep{Fernandez-Arenas2007}, como desde el enfoque de células
del sistema inmune del hospedador como macrófagos \citep{Reales-Calderon2014, Reales-Calderon2013};
estudios del proteoma de la superficie celular y proteínas secretadas \citep{Gil-Bona2015, Gil-Bona2015a},
del proteoma de la membrana plasmática \citep{Cabezon2009}, de la pared celular \citep{Pitarch2002, Castillo2008} 
o del proteoma implicado en la transición dimórfica levadura-hifa \citep{Monteoliva2010}.
También se han realizado aproximaciones proteómicas para la búsqueda de biomarcadores
de diagnóstico y pronóstico de candidiasis \citep{Pitarch2006b, Pitarch2011}. Y sin embargo, a pesar de la extensa variedad de estudios proteómicos abordados, la presencia
de resultados en respositorios públicos \textit{online} era hasta hace relativamente poco tiempo
escasa y en ocasiones poco fiable. Las herramientas bioinformáticas descritas en esta tesis
contribuyen a mejorar y ampliar el catálogo existente de resultados de estudios proteómicos
que utilizan \textit{\mbox{C. albicans}} en diferentes condiciones.
...
\subsection{\textit{Candida} Genome Database, CGD. Base de datos del genoma de \textit{Candida}}
\label{CGD}
La base de datos \ac{CGD}, creada en la universidad de Stanford, California
es el recurso de referencia empleado por la comunidad científica para obtener información y secuencias de {Candida spp.} \citep{Inglis2012}.

El ensamblaje de su secuencia genómica descrito en \citet{Muzzey2013} fue el
primer ensamblaje diploide, con información de alelos, descrito y recogido en CGD.

Las secuencias de proteínas traducidas a partir de dicho ensamblaje se han usado para
construir las bases de datos empleadas en estudios proteómicos descritos en esta tesis como \citet{Vialas2015a}.

Además las herramientas de enriquecimiento y mapeo de anotaciones de términos GO, GO Term Finder y
GO Slim Mapper, accesibles a través de CGD, han sido extensamente empleadas en \citet{Vialas2013} y \citet{Vialas2015a}.
