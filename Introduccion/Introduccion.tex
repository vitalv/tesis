...
En ocasiones, como es en el caso del ESI, el sistema
...
de \emph{tiempo de vuelo} o una \emph{trampa iónica} para formar un QTOF
...
{Cuadrupolo, Trampa Iónica Tridimensional(QIT) y Orbitrap}


La \textit{sensibilidad}, \textit{resolución} y \textit{precisión}
son parámetros importantes en un espectrómetro de masas 
 ya que determinan 
...
el péptido precursor es seleccionado para ser fragmentado en base a la 
intensidad en el espectro MS\textsuperscript{1} se denomina por eso
Adquisición Dependiente de Datos (\emph{Data Dependent Acquisition, DDA})
...
Los iones más abundantes e informativos son los \emph{b-} e \emph{y-},
 generados por la fragmentación en el enlace peptídico entre aminoácidos, 
el punto de menor energía de la estructura.
En analizadores tipo cuadrupolo o cuadrupolo-TOF predominan los iones \emph{y-},
...
La \emph{Disociación Inducida por Colisión} (CID) 
...
En la \emph{Disociación por 
Transferencia Electrónica} (ETD), \citep{Syka2004}, los iones de analito
...
una fragmentación en la que las cadenas laterales y modificaciones 
post-traduccionales quedan intactas.
Estas cualidades hacen ETD interesante para proteómica \emph{top-down},
que intenta identificar proteínas intactas, y también
para experimentos orientados a la deteccion de péptidos con modificaciones
post-traduccionales.


En la \emph{Disociación por Captura Electrónica} (ECD) \citep{Zubarev1998},
como en ETD, la fragmentación se produce por la interacción del analito
con electrones que, en este caso, son suministrados por introducción directa.
 ECD se usa generalmente en espectrómetros
tipo FTICR y en trampas iónicas y genera abundantes fragmentos de series
...
Otro tipo de fragmentación es la \emph{Disociación por Alta Energía de Colisión} (HCD)
usado en analizadores tipo Orbitrap \citep{Olsen2007}. Al igual que CID
...
proteína con modificaciones post-traduccionales, lo que difícilmente se puede
...
ellas que pueden tener masas muy similares. Por ese motivo,
 ...
 generalmente a partir de una \textit{mancha} o \textit{spot} proteico de 2D-PAGE
...
proteómica de alto rendimiento o \textit{shotgun}. Como se describe a
continuación, la identificación
...
contando con que a partir de cada proteína, se generan múltiples
...
electroforesis en un gel 1D-PAGE,
por fraccionamientos sub-celulares correspondientes a distintos 
orgánulos, etc... 

Pero el fraccionamiento más importante se hace a nivel de péptido, por cromatografía
...
se les denomina generalmente PSM (\textit{Peptide Spectrum Match})

Las estrategias utilizadas para la asignación de PSMs básicamente pueden
 clasificarse en tres tipos. La más extendida es la búsqueda utilizando
...
Los motores de búsqueda, descritos en detalle en una sección posterior,
...
y las secuencias de las proteínas en una base de datos de referncia.
...
la masa del péptido. Sin embargo
...
Algunas de éstas ocurren con una frecuencia variable, como las
oxidaciones que se pueden producir en algunas de las metioninas (M).
Otro tipo de modificaciones son las llamadas fijas, como la carbamidometilación
en las cisteínas (C), un artefacto que se produce en todas las C
para evitar la formación de puentes di-sulfuro.
La selección de este tipo de modificaciones permite que el motor de búsqueda
pueda tener en cuenta un posible desfase de masa en el péptido, equivalente
por ejemplo, a un átomo de oxígeno o un grupo carbamido-metil en el
caso de las modificaciones descritas.
...
Así, en instrumentos de tipo cuadrupolo o híbridos cuadrupolo-TOF,
...
asignar a cada espectro obtenido un péptido, el mejor candidato de una lista de los
...
{Estrategia básica de identificación. Correlación espectro MS/MS - secuencia aminoacídica}
...
calcula la puntuación de correlación (\emph{cross-correlation Score, Xcorr})
para todos los candidatos con los que se ha comparado.
En primer lugar se crea un espectro procesado (espectro X) en el que los picos de baja 
...
Ambos valores son por tanto indicativos de la calidad de cada PSM que será
mejor cuanto más altas sean ambas puntuaciones.

%X!TANDEM%%%
Otro motor de búsqueda frecuentemente utilizado es \emph{X!Tandem} \citep{Craig2004},
...
pero en este caso se tiene en cuenta si los iones coincidentes pertenecen
...
\phantomsection
\subsection*{Búsqueda mediante etiquetas de secuencia}
\addcontentsline{toc}{subsection}{Búsqueda mediante etiquetas de secuencia}
...
\phantomsection
\subsection*{Búsquedas tolerantes y multi-etapa}
\addcontentsline{toc}{subsection}{Búsquedas tolerantes y multi-etapa}
...
todos los PSMs en el conjunto del experimento.
...
péptidos en la lista de candidatos creada a partir de la base de datos de referencia, 
se puede obtener una lista de péptidos ordenados en función 
de la puntuación obtenida que mide el grado de similitud entre el espectro
empírico adquirido y el teórico.
...
Para ello, en primer lugar se selecciona el mejor péptido 
asignado a un espectro, es decir aquel candidato con la mejor puntuación,
%. La \emph{hipótesis nula} se estima construyendo
y a continuación se construye
una distribución de las puntuaciones de todo el resto de péptidos comparados
con el espectro. Esta distribución 
representa la hipótesis nula, una asignación PSM por azar.
...
de que el mejor péptido candidato seleccionado sea asignado por azar al espectro.
...
para un PSM en concreto) si se evalúan miles de espectros MS/MS podrían 
ocurrir PSMs con \emph{p-valores} igualmente bajos solo por azar. Por este
motivo se utilizan estrategias de \emph{corrección de test múltiples} 
...
Así para un PSM con \emph{p-valor} = 0,05 en un experimento en el que hay
otros 10.000 PSMs, el \emph{p-valor} original habría de reajustarse a
...
de todos los PSMs de un experimento y permiten por tanto estimar otros parámetros
...
Básicamente, el concepto de tasa FDR se refiere a la proporción de PSMs incorrectos
que se aceptan en todo el conjunto de PSMs de un experimento para un umbral
...
esencialmente en utilizar una base de datos señuelo \citep{Elias2007}.
Es una aproximación sencilla pero efectiva que requiere que los espectros MS/MS
sean comparados con espectros teóricos derivados de secuencias llamadas \emph{señuelo} o 
\emph{decoy}, que pueden ser construidas de varias formas pero que, en cualquier
caso, son secuencias que no existen, no corresponden a ninguna proteína. 
Para ello, se puede añadir a la base de datos de secuencias reales 
(secuencias \emph{target})
...
El objetivo de la creación de estas secuencias \emph{señuelo} es % permiten %por tanto
recrear una hipótesis nula. Sabemos que los resultados de identificaciones
correspondientes a secuencias \emph{señuelo}, claramente etiquetadas
en el fichero fasta, son identificaciones incorrectas. Con esta hipótesis nula, la estrategia asume una idea básica central: la frecuencia con que los
espectros MS/MS son asignados a secuencias \emph{señuelo} sigue la 
misma distribución que la frecuencia con que los espectros son asignados
incorrectamente a secuencias \emph{target}. 

Así, de forma general y dado que las bases de datos de secuencias 
\textit{target} y \textit{señuelo}
tienen el mismo tamaño, el número de PSMs incorrectos
(N\textsubscript{inc}, aquellos espectros a los que se ha asignado
incorrectamente una secuencia \textit{target})
puede ser considerado equivalente al número de PSMs \emph{señuelo} 
...
esto es, la proporción de PSMs \textit{señuelo}, N\textsubscript{d} como sustituto
...
En ocasiones, de forma alternativa, la tasa FDR también puede calcularse como 
2N\textsubscript{d}/(N\textsubscript{t}+N\textsubscript{d})

Esta estimación genérica puede tener variantes.
...
Una forma de corregir esto consiste en estimar una aproximación 
previa de la fracción N\textsubscript{inc} dentro de N\textsubscript{t} \citep{Kall2008}.  
Así se puede incluir en la tasa FDR un factor de corrección definido
por el porcentaje estimado de PSMs \textit{target} incorrectos (PIT): Si en 
N\textsubscript{t} el 80\% de los PSMs son incorrectos, la tasa FDR 
calculada como N\textsubscript{d}/N\textsubscript{t}, se multiplica por 0.8
para obtener un valor FDR más preciso (Por cada 100 PSMs \textit{señuelo}
en el conjunto de PSMs aceptado se estiman 80 PSMs \textit{target} incorrectos) 

Otra forma de corregir la sobre-estimación de de N\textsubscript{inc}
usando búsquedas separadas consiste en e

Cuando se hacen búsquedas utilizando una base de datos concatenada 
\textit{target-decoy} también se produce un resultado conservador. En este caso
se debe al efecto de competición entre secuencias \textit{target} y señuelo.
Esto significa que a algunos espectros se les puede asignar una secuencia
señuelo con una puntuación mayor a la que se obtiene al asignarles la
secuencia \textit{target} real. En tal caso se produce una reducción del
número de PSMs correcto y por tanto un incremento de FDR


%This PIT is equivalent to the ratio of the area under the dotted black line (the incorrect target PSMs) to the area under the solid black line (the decoy PSMs).
%The PIT is important because it allows us to reduce the estimated FDR associated with a given set of accepted target PSMs. In our simulation, if we accept X decoy PSMs with scores above a certain threshold, then we expect to find 0.8 X incorrect target PSMs above the same theshold. A more accurate estimate of the FDR, therefore, is to multiply the previous estimate -the ratio of the number of decoy PSMs versus target PSMs above a threshold- by the PIT.



%En cuanto a la forma en que se construyen estas secuencias señuelo
...
\textit{
Dice Elias Gygi que "Separate searching overestimates FP rates by underestimationg low-scoring correct PSMs
Pero la estrategia de J. Vazquez son búsquedas separadas
}
...
cada PSM individual. % en el conjunto de todos los PSMs del experimento.
Esto se puede calcular mediante la \emph{Probabilidad de Error Posterior, PEP},
de un PSM, que se define como la fracción
estimada de PSMs correctos entre una colección de PSMs con puntuaciones 
...
promedio de puntuaciones de todos los PSMs del experimento es una combinación
de distribuciones, un modelo mixto de probabilidad, que integra la 
distribución de los PSMs incorrectos y la distribución de los PSMs correctos.
