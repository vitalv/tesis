...
Las estrategias utilizadas para la asignación de PSMs básicamente pueden
 clasificarse en tres tipos. La más extendida es la búsqueda basada en secuencia,
...
una fracción son correctos, es decir corresponden realmente a la secuencia
...
mayor es la significatividad estadística de la asignación PSM.
...
de un experimento. En este caso, las medidas estadísticas de las distribuciones
de espectro individual no son suficientes. Incluso en el caso de que se requiera
...
motivo se utilizan estrategias de \emph{corrección de tests múltiples} 
...
estadísticos adicionales a nivel global, como la Tasa de Falsos Positivos, FDR
y la Probabilidad de Error Posterior, PEP.


\subsubsection*{Bases de datos señuelo, Tasa de Falsos Descubrimientos (FDR)
y Probabilidad de Error Posterior (PEP)}

En el caso de experimentos de alto rendimiento en los que se producen
miles de espectros, la evaluación estadística más ampliamente utilizada
es un tipo de corrección de tests múltiples, 
...
Para la estimación de la tasa FDR, la estrategia utilizada generalmente
consiste en utilizar una base de datos señuelo \citep{Elias2007}.
Es una aproximación sencilla y efectiva que requiere que los espectros MS/MS
sean comparados con secuencias llamadas \emph{señuelo} o 
...
use esta base de datos concatenada (\emph{target-decoy}) del doble de tamaño que la original.
 O bien se pueden realizar dos búsquedas consecutivas, una utilizando las 
secuencias \emph{reales} y otra a continuación utilizando las \emph{señuelo}.

El objetivo de la creación de estas secuencias \emph{señuelo} es % permiten %por tanto
recrear una hipótesis nula. Sabemos que los resultados
correspondientes a proteínas señuelo (dado que están claramente etiquetadas
en el fichero fasta) son identificaciones incorrectas.

Con esta hipótesis nula, la estrategia asume una idea básica central: la frecuencia con que los
...
incorrectamente a secuencias \emph{reales}. 
...
%Elias Gygi, Nat Method \citep{Elias2007}
...
%
%SALVA:
%Algunos motores de búsqueda como X!Tandem, Phenyx, o Mascot
%utilizan una
...
%mismo experimento, es esencial conocer también la distribución global de las asignaciones de
